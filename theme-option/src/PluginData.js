  ...
  function PluginList(props){
    ...
    const renderDataa = Object.keys(item).map((items) => {
      ...
      if(item[items].status == 'pro-installed'){
        ...
        nameTxt = <h4>{item[items].name}<span>{__( 'Pro', 'blur' )}</span></h4>
        proDiv      =<a target="_blank" className="th-optn-link th-view-m" href={item[items].link}> {__( 'View More', 'blur' )}</a>;;
        ...
        pStatusInst = item[items].status;
          ...
          proDiv  = <a target="_blank" className="th-optn-link th-go-pro" href={item[items].link}> {__( 'Go Pro', 'blur' )}</a>;
