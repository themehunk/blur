<?php 
  ...
  function blur_pro_free_check($key, $free_init, $pro_init){
...
function blur_theme_option_endpoint_callback() {
   ...
   $request_json = get_template_directory() . '/theme-option/data.json';
    ...
    foreach ($data as $key => $value) {
      ...
      $plugin_status = blur_pro_free_check($key, $value->init, $value->proInit);
...
if (! is_plugin_active($plugin_init) && $instl == 'install-now') {
  ...
  if ( ! current_user_can( 'install_plugins' ) ) {
		$status['errorMessage'] = __( 'Sorry, you are not allowed to install plugins on this site.','blur' );
